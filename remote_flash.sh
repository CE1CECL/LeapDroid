...
nand_flash_bulk () {
  ...
  if [[ $prefix == lf2000_* ]]; then
    ...
    ${SSH} "echo \"    write /sys/devices/platform/lf2000-touchscreen/pointercal \\\"$(${SSH} chroot /mnt/rfs /usr/bin/mfgdata get ts)\\\"\" >> \"/mnt/bulk/init.nxp3200.rc\""
    ${SSH} "echo \"    write /sys/devices/platform/lf2000-aclmtr/calibration \\\"$(${SSH} chroot /mnt/rfs /usr/bin/mfgdata get aclcal)\\\"\" >> \"/mnt/bulk/init.nxp3200.rc\""
    ${SSH} "echo \"    write /sys/devices/platform/lf2000-power/adc_constant \\\"$(${SSH} chroot /mnt/rfs /usr/bin/mfgdata get adc | cut -d ' ' -f 1)\\\"\" >> \"/mnt/bulk/init.nxp3200.rc\""
    ${SSH} "echo \"    write /sys/devices/platform/lf2000-power/adc_slope_256 \\\"$(${SSH} chroot /mnt/rfs /usr/bin/mfgdata get adc | cut -d ' ' -f 2)\\\"\" >> \"/mnt/bulk/init.nxp3200.rc\""
    ${SSH} "echo \"    write /sys/devices/platform/lf2000-touchscreen/tails \\\"$(if [ $(${SSH} chroot /mnt/rfs /usr/bin/mfgdata get tsp | grep \"Version=\" | cut -d = -f 2) -lt 4 ]; then echo \"1\"; else echo \"0\"; fi)\\\"\" >> \"/mnt/bulk/init.nxp3200.rc\""
