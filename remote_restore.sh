...
restore_nand () {
  ...
  nand_part_detect
  for mtd in $(ssh root@169.254.8.1 "ls /dev/mtd*"); do echo $mtd; ubi=$(basename $mtd); echo $ubi; dd status=progress if=$prefix$ubi | ssh root@169.254.8.1 "dd of=$mtd"; done
...
restore_mmc () {
  ...
  ${SSH} -o "StrictHostKeyChecking no" 'test'
  for mtd in $(ssh root@169.254.8.1 "ls /dev/mmc*"); do echo $mtd; ubi=$(basename $mtd); echo $ubi; dd status=progress if=$prefix$ubi | ssh root@169.254.8.1 "dd of=$mtd"; done
  echo "Done! Rebooting your LeapFrog Device."
  ${SSH} "(echo 1 >/proc/sys/kernel/sysrq) && (echo b >/proc/sysrq-trigger)"
}
